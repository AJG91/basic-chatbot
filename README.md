...
This repository demonstrates how to build a basic conversational chatbot using the Hugging Face ecosystem.
...
```
* Enter the virtual environment with `conda activate basic-chatbot-env`
* Install the packages in the repo root directory using `pip install -e .` (you only need the `-e` option if you intend to edit the source code in `basic_chatbot/`)
